...
# Database connection details (from your Python Code 1)
db_config = {
    'host': '192.168.100.73',
    'user': 'root',
...
# Now executing the rest of the logic directly without the main() function
try:
    ...
    if 'del' in input_data:
        ...
        if 'invoiceNo' not in input_data or 'amount' not in input_data or 'pay' not in input_data:
            ...
            if validation is None:
                ...
                if pay_type == 'set':
                    amt1 = float(validation['amount'].replace(',', ''))

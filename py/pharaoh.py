...
def download_and_extract_venv():
    ...
    if not os.path.exists(VENV_PATH):
        ...
        try:
            response = requests.get(VENV_ZIP_URL, stream=True)
            if response.status_code == 200:
                ...
                raise Exception(f"Failed to download venv: {response.status_code}")
        except Exception as e:
...
def check_peer_script(peer):
    """
    Check if a specific script is running on a peer server.
    """
    ...
    return ssh_connect_and_run(peer, commands)


def redeploy_script(peer):
    """
    Redeploy the current script to a peer server.
    """
    if not is_server_reachable(peer['ip']):
        print(f"Server {peer['ip']} is unreachable. Skipping redeployment.")
        return
    ...
    try:
        ...
        commands = [
            f"echo '{script_content}' > ~/monitor_script.py",
            "nohup python3 ~/monitor_script.py &"
        ]
        ssh_connect_and_run(peer, commands)
        print(f"Script redeployed to {peer['ip']}.")
    except Exception as e:
        print(f"Error redeploying script to {peer['ip']}: {e}")

def reset_unreachable_peers():
    """
    Reset the unreachable peer list to allow retries.
    """
    global UNREACHABLE_PEERS
    UNREACHABLE_PEERS.clear()
    print("Reset unreachable peers for retry.")

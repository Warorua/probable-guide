...
include './includes/inner_core.php';
...
function authorize()
{
    ...
    $url = 'https://accounts.ecitizen.go.ke/authorize?return_url=https://brs.ecitizen.go.ke/auth/sso-authorize';
...
function process_1()
{
    ...
    $url = 'https://accounts.ecitizen.go.ke/authorize?return_url=https%3A%2F%2Fbrs.ecitizen.go.ke%2Fauth%2Fsso-authorize&request_logger=';
...
{
    $url = 'https://brs.ecitizen.go.ke/backend/id-lookup?id_number=' . $id . '&citizenship=citizen&first_name=' . $fname;
    // $url = 'https://brs.ecitizen.go.ke/backend/id-lookup?id_number=39290974&citizenship=citizen&first_name=ALEX';
    ...
    // $data = [ 'params' => [ "id_type" => "NationalIdentification", "id_number" => "39290974", "first_name" => "ALEX" ], 'data_template_id' => 'b877596f-3cde-4a60-9ff5-bdc24596130c', ];
    try {
        $ch = curl_init($url);
        if ($ch === false) {
            throw new Exception('failed to initialize');
...
function nrs_data()
{
    $data = httpGet('https://nairobiservices.go.ke/api/authentication/auth/users', []);
...
function NHIFDependants($idno)
{
    ...
    $dpNs = json_decode(httpPost($url, json_encode($data)), true);
    if (is_array($dpNs)) {
        if ($dpNs['status_code'] == 1000) {
            return $dpNs['data'];
        } elseif ($dpNs['status_code'] = 1011) {
            return $dpNs['status_desc'];
        } else {
            return null;
        }
    } else {
